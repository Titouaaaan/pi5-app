...
export default function Timeline() {
  return (
    <section className="flex flex-col gap-4">
      ...
      <dl className="grid grid-cols-[88px_minmax(0,1fr)] gap-x-[18px] gap-y-3.5 text-[15px] leading-snug sm:grid-cols-[132px_minmax(0,1fr)]">
        {timeline.map((entry) => (
          <div key={`${entry.date}-${entry.role}`} className="contents">
            <dt className="font-mono text-[14px] text-faint">{entry.date}</dt>
